...
const Select = ({
    ...
    return (
        <div className={className || ""}>
            ...
            <ListboxButton
                    className={`group relative flex justify-between items-center w-full h-12 pl-4.5 pr-3 border border-s-stroke2 rounded-3xl text-body-2 text-t-primary fill-t-secondary transition-all data-[hover]:border-s-highlight data-[hover]:text-t-primary data-[open]:text-t-primary data-[open]:rounded-b-none data-[open]:border-s-subtle data-[open]:border-b-transparent ${
                    isBlack
            ...
            <ListboxOptions
                ...
                anchor={forceDownward ? "bottom start" : "bottom"}
                modal={false}
                transition
            >
                {options.map((option) => (
            ...
            {error && <div className="mt-2 text-caption text-red-500">{error}</div>}
        </div>
    );
};
